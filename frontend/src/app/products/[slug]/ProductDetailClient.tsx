...
Can you provide more details about this product?`
  ...
  const addToCartMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({ title: err.response?.data?.detail || 'Failed to add to cart', variant: 'destructive' });
    },
  });
  const addToFavoriteMutation = useMutation({
    mutationFn: () => api.addFavorite(product!.id, guestSessionId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['favorites'] });
      toast({ title: 'Added to favorites', variant: 'success' });
    },
    onError: (err: any) => {
      toast({ title: err.response?.data?.detail || 'Failed to add to favorites', variant: 'destructive' });
    },
  });
            ...
            <div className="pt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  variant="outline"
                  className={cn(
                    'h-11 w-full px-3 transition-colors',
                    isInCart &&
                ...
                >
                  {isInCart ? (
                    <>
                      ...
                      <span className="ml-2 font-medium flex items-center gap-2"> Added to Cart </span>
                    </>
                  ) : addToCartMutation.isPending ? (
                    <span className="font-medium">Adding…</span>
                  ) : (
                    <span className="font-medium flex items-center gap-2"> <ShoppingCart className="w-5 h-5" />Add to Cart </span>
                  )}
                </Button>
                ...
                >
                  <Button
                    size="lg"
                    variant="outline"
                    className="h-11 w-full border-green-500/40 text-green-600 hover:bg-green-500/10 hover:text-green-700 dark:text-green-400 px-3"
                  >
                    ...
                    <span className="truncate text-sm font-medium">WhatsApp</span>
                  </Button>
                </Link>
              </div>
            </div>
